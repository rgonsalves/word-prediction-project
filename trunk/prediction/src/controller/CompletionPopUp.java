...
public abstract class CompletionPopUp {
	...
	static Action acceptAction = new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			...
			CompletionPopUp completer = (CompletionPopUp) tf
					.getClientProperty(COMPLETION);
	...
	public void showPopup() {
		...
		if (updateListData() && list.getModel().getSize() != 0) {
			...
			for (int i = 0; i < size; i++) {// KeyEvent.VK_F1
				textComp.registerKeyboardAction(acceptAction, "F" + (i + 1),
						KeyStroke.getKeyStroke(112 + i, 0),
						JComponent.WHEN_FOCUSED);
			}
			...
			popup.show(textComp, x + 10, y + 13); // popup.show(textComp, 0,
													// 230);
		} else {
			popup.setVisible(false);
		}
