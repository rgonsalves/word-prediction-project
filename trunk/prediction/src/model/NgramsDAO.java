...
public class NgramsDAO {
	...
	public static void recreateDb() throws Exception {
		NgramsDAO.clearNgrams();
		Ngrams.buildNgrams();
	}
	...
	private boolean checkPatternWords(int n, String pattern, String word,
	...
	public Vector<String> getRowsByPattern(int n, String pattern)
			throws Exception {
		String sql = " SELECT * FROM NGRAM_" + n + " WHERE PATTERN ='"
				+ pattern + "' ORDER BY RECEIVER, FREQUENCY DESC";
		connection.executeQuery(sql);
		NgramVO ngram = null;
	...
	private void updateDbFrequency(int n, int frequency, int id)
			throws Exception {
		String sql = " UPDATE NGRAM_" + n + " SET FREQUENCY=" + (frequency)
				+ " WHERE ID =" + id;
		connection.execute(sql);
	}
	...
	public void getNgrams(String pattern, String word, String receiver, int n)
		...
		if (checkPatternWords(n, pattern, word, frequency, receiver)) {
			ngram = NgramVO.getNgramDb(connection);
			if (!ngram.getPattern().equals(pattern)
					|| !ngram.getWord().equals(word)
				...
				if (ngram.getPattern().equals(pattern)
						&& ngram.getWord().equals(word)
						&& ngram.getReceiver().equals(receiver)) {
					updateDbFrequency(n, ngram.getFrequency() + 1,
							ngram.getId());
				}
			}
		} else
