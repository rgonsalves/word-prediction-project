...
/**
 * Clase generica que empaqueta todas las acciones relativas a la conexion y
 * ejecucion de consultas en la BD.
 */
public abstract class GenericWrapper {
	...
	/**
	 * For preparing the query execution, closing the Resultset or the
	 ...
	 * 
	 * @exception Exception
	 *                if any problem during the closing
	 */
	 ...
	 * 
	 * @exception Exception
	 *                if there is any problem trying to close PreparedStatement
	 */
				...
				while ((countdown != 0) && (requete_executee == false)) {
					try {
						...
						if (messageb.indexOf("ORA-08177") != -1) {
							...
							throw new Exception("Error during de SQL query."
									+ error.toString());
						}
					}
				}
				if (requete_executee == false) {
					throw new Exception("Connection error during de SQL query."
							+ e.toString());
				}
			} else {
				throw new Exception("Connection error during de SQL query"
	 ...
	 * Cierra la conexi_n a la base de datos si no se hace todav_a. El metodo
	 * cerrar se debe llamar expl_citamente sobre los objetos JDBCWrapper en la
	 * cl_usula final.
	 */
	...
	/**
	 * Obtiene la fecha de un campo mediante el m_todo getTimeStamp. El tipo que
	 * devuelve es java.util.Date El metodo que lo llame podr_ hacer casting a
	 * java.sql.timestamp
	 * 
	 * @param champ
	 *            field name
	...
	/**
	 * Obtiene la fecha de un campo mediante el m_todo getTimeStamp. El tipo que
	 * devuelve es java.util.Date El metodo que lo llame lo llame podr_ hacer
	 * casting a java.sql.Date
	 * 
	 * @param champ
	 *            field name
	 * @return field value
	 * @exception Exception
	 *                Si hay algun error leyendo el Resulset
	 */
	...
	public long getLong(String champ) throws Exception {
		try {
			...
			throw new Exception("Error de lectura sobre el campo [{0}]."
					+ champ + " " + e.toString());
		}
	}
	...
	public String getClob(String champ) throws Exception {
		try {
			if (m_Rset.getClob(champ) != null)
				return m_Rset.getClob(champ).getSubString(
						1,
						new Integer("" + m_Rset.getClob(champ).length())
								.intValue());
			else
				return "";
		} catch (Exception e) {
